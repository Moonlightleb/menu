/* ── KSS_ENGINE boot ─────────────────────────────────── */
KSS_ENGINE.init({
  serviceId: "66d739121389bc24cc9d5407",
  type: "menu",
  filterEmpty: true,
...
KSS_ENGINE.onReady(function (menuData) {
  ...
  if (loaderLogo) loaderLogo.textContent = STORE.storeName;
...
function buildHeader(STORE) {
  menuHeader.innerHTML = `
    ...
    ${
      (STORE.sm && STORE.sm.instagramUrl) || (STORE.sm && STORE.sm.facebookUrl)
        ? `
            <div class="details-insta-fb">
              ...
              ${
                STORE.sm.facebookUrl && STORE.sm.instagramUrl
                  ? `
                      <a
                        class="a-white"
                        href="${STORE.sm.facebookUrl}"
    ...
    ${
      STORE.phoneNumber
        ? `
            <div class="details">
              <a href="tel:${STORE.phoneNumber}" class="a-white">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="16"
                  height="16"
...
function buildWhatsappButton(STORE) {
  if (STORE.sm && STORE.sm.whatsappUrl) {
    ...
    wp.id = "wp-btn";
    wp.innerHTML = `
      <a href="${STORE.sm.whatsappUrl}" target="_blank">
        <svg
          xmlns="http://www.w3.org/2000/svg"
          width="16"
          height="16"
...
function displayMenu(subCategories, STORE) {
  ...
  subCategories.forEach((sub) => {
    menuBtnDOM.innerHTML += `
      <button onclick="click_Tag('${sub._id}')">
        <a style="display: none" class="reference" href="#${sub._id}"></a>
        <div>${sub.label}</div>
      </button>`;
    ...
    section.innerHTML += `
      <div class="section-header">
        ${sub.bgImg ? `<img src="${sub.bgImg}" alt="${sub.label}"/>` : ""}
        <p>${KSS_ENGINE.capitalizeFirstLetter(sub.label)}</p>
      </div>`;

    const article = document.createElement("article");
    section.appendChild(article);
    sub.items.forEach((item) => {
      const price = KSS_ENGINE.formatPrice(item.price, STORE.currency);
      article.innerHTML += `
        <div class="item">
...
// Show and hide whatsapp and back to top links
